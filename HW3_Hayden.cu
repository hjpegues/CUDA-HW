...
/*
 ...
 Line 52: Capitalized the C
 Line 73: Changedf from 101 to 256 
 Line 78: Replaced 1 with our equation from class
 Line 122: Added block id & size to our thread id
 Line 125: Replaced the while loop with an if statement
*/
...
void CleanUp(); //Capitalized the C
...
void setUpDevices()
{
	BlockSize.x = 256; //Chaged from 101 to 256
	BlockSize.y = 1;
	BlockSize.z = 1;
	
	GridSize.x = ((N-1)/BlockSize.x) + 1; //Put in our equation from class
	GridSize.y = 1;
	GridSize.z = 1;
}
...
{
	int id = blockIdx.x * blockDim.x + threadIdx.x; //Added blockId * blockDim to our threadId
	
	if(id < n) //Replaced the while loop with this if statement
	{
		c[id] = a[id] + b[id];
	}
}
